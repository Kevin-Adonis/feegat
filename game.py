...
class Game(object):
    def __init__(self, config, env, random_seed=1000):
        ...
        self.data_dir = singleton.data_dir
        self.DG = singleton.DG
        self.traffic_file = singleton.traffic_file
        self.traffic_matrices = singleton.traffic_matrices
        self.traffic_matrices_dims = singleton.traffic_matrices.shape
        self.tm_cnt = singleton.tm_cnt
        self.num_pairs = singleton.num_pairs
        self.idx2sd = singleton.idx2sd
        self.sd2idx = singleton.sd2idx
        self.num_nodes = singleton.num_nodes
        self.num_links = singleton.num_links
        self.idx2edge = singleton.idx2edge
        self.edge2idx = singleton.edge2idx
        self.link_capacities = singleton.link_capacities
        self.link_weights = singleton.link_weights
        self.shortest_paths_node = singleton.shortest_paths_node              # paths with node info
        self.shortest_paths_link = singleton.shortest_paths_link              # paths with link info
        ...
        self.lp_links = [e for e in self.edge2idx]
        self.pair_links = [(pr, e[0], e[1]) for pr in self.lp_pairs for e in self.lp_links]

        self.load_multiplier = {}

        self.edge2sdi = singleton.edge2sdi
        self.link_capacities_map = singleton.link_capacities_map
        self.link_weights_map = singleton.link_weights_map
    ...
    def get_ecmp_next_hops(self):
        ...
        for src in range(self.num_nodes):
            for dst in range(self.num_nodes):
                ...
                for p in self.shortest_paths_node[self.sd2idx[(src, dst)]]:
    ...
    def ecmp_next_hop_distribution(self, link_loads, demand, src, dst):
        ...
        #if next_hops_cnt > 1:
            #print(self.shortest_paths_node[self.sd2idx[(src, dst)]])

        ecmp_demand = demand / next_hops_cnt 
        for np in ecmp_next_hops:
            link_loads[self.edge2idx[(src, np)]] += ecmp_demand
    ...
    def optimal_preconfigured_path_routing_mlu(self, tm_idx):

        K = singleton.K

        tm = {}
        for row_index, row in enumerate(self.traffic_matrices[tm_idx]):
            for column_index, element in enumerate(row):
                if row_index!=column_index:
                    tm[(row_index,column_index)] = element

        model = LpProblem(name="routing")

        ridx = [(i, j, k) for k in range(K) for i in range(self.num_nodes) for j in range(self.num_nodes) if i!=j]
        ratio = LpVariable.dicts(name="ratio", indices=ridx , lowBound=0.0, upBound=1.0)

        link_load = LpVariable.dicts(name="link_load", indices=self.lp_links)
        #link_score = LpVariable.dicts(name="link_score", indices=self.lp_links)

        r = LpVariable(name="congestion_ratio")
        g1 = LpVariable(name="g1")
        g2 = LpVariable(name="g2")

        for i in range(self.num_nodes):
            for j in range(self.num_nodes):
                if i!=j:
                    model += lpSum(ratio[(i,j,k)] for k in range(K))==1

        for sd in self.lp_links:
            model += (link_load[sd] == lpSum([ratio[(s,d,i)]*tm[(s,d)] for s,d,i in self.edge2sdi[sd]]))

        for sd in self.lp_links:
            model += (link_load[sd] <= self.link_capacities_map[sd]*r)

        model += ( g1 == r )
        model += ( g2 == lpSum([link_load[sd]*(1/self.link_capacities_map[sd]) for sd in self.lp_links]))
        model += g1 + 0 * g2
        ...
        res_link_load = np.array([link_load[i].value()*(1/self.link_capacities_map[i])for i in link_load])

        # plt.clf()
        # g = sns.heatmap(graph, cmap='viridis',vmin=0, vmax=0.05)
        # plt.show()
        # plt.savefig('heatmap_feegat.png')
        # y = self.num_nodes*(self.num_nodes-1)

        idx = 0
        y = np.zeros((self.num_nodes*self.num_nodes-self.num_nodes,singleton.K))
        for i in range(self.num_nodes):
            for j in range(self.num_nodes):
                if i!=j:
                    for k in range(singleton.K):
                        y[idx][k] = ratio[(i,j,k)].value()
        ...
        # G = singleton.DG
        # plt.clf()
        # nx.draw(G, pos=nx.spring_layout(G),with_labels=True,edge_color = [G[u][v]['load'] for u,v in G.edges()])
        # plt.show()
        # plt.savefig('network_structure.png')
        return g1.value() , y , res_link_load
    ...
    def optimal_routing_mlu(self, tm_idx):
        ...
        ratio = LpVariable.dicts(name="ratio", indices=self.pair_links, lowBound=0, upBound=1)
        link_load = LpVariable.dicts(name="link_load", indices=self.links)
        r = LpVariable(name="congestion_ratio")

        for pr in self.lp_pairs:
            model += (lpSum([ratio[pr, e[0], e[1]] for e in self.lp_links if e[1] == self.idx2sd[pr][0]]) - lpSum([ratio[pr, e[0], e[1]] for e in self.lp_links if e[0] == self.idx2sd[pr][0]]) == -1, "flow_conservation_constr1_%d"%pr)

        for pr in self.lp_pairs:
            model += (lpSum([ratio[pr, e[0], e[1]] for e in self.lp_links if e[1] == self.idx2sd[pr][1]]) - lpSum([ratio[pr, e[0], e[1]] for e in self.lp_links if e[0] == self.idx2sd[pr][1]]) == 1, "flow_conservation_constr2_%d"%pr)

        for pr in self.lp_pairs:
            for n in self.lp_nodes:
                if n not in self.idx2sd[pr]:
        ...
        #     ei = self.edge2idx[e]
        #     a,b = e
        #     lls.append(link_load[ei].value()/self.link_capacities[ei])
       
        # plt.clf()
        # g = sns.heatmap(graph, cmap='viridis',vmin=0, vmax=0.05)
        # plt.show()
        # plt.savefig('heatmap_mlu.png')
    ...
    def eval_optimal_routing_mlu(self, tm_idx, solution, eval_delay=False):
        ...
        return optimal_max_utilization, delay , optimal_link_loads/self.link_capacities
    ...
    def optimal_routing_mlu_critical_pairs(self, tm_idx, critical_pairs):
        ...
        ratio = LpVariable.dicts(name="ratio", indices=pair_links, lowBound=0, upBound=1)
    ...
    def optimal_routing_delay(self, tm_idx):
        ...
        f = LpVariable.dicts(name="link_cost", indices=self.links)
...
class CFRRL_Game(Game):
    def __init__(self, config, env, random_seed=1000):
        ...
        self.action_dim = singleton.num_pairs
    ...
    def evaluate(self, tm_idx, actions=None, ecmp=True, eval_delay=False):
        if ecmp:
            ecmp_mlu, ecmp_delay,ecmp_link_loads = self.eval_ecmp_traffic_distribution(tm_idx, eval_delay=eval_delay)
        ...
        crit_mlu, crit_delay ,crit_link_loads= self.eval_critical_flow_and_ecmp(tm_idx, crit_topk, solution, eval_delay=eval_delay)

        topk = self.get_topK_flows(tm_idx, self.lp_pairs)
        _, solution = self.optimal_routing_mlu_critical_pairs(tm_idx, topk)
        topk_mlu, topk_delay , topk_link_loads = self.eval_critical_flow_and_ecmp(tm_idx, topk, solution, eval_delay=eval_delay)

        _, solution = self.optimal_routing_mlu(tm_idx)
        optimal_mlu, optimal_mlu_delay , opti_link_loads = self.eval_optimal_routing_mlu(tm_idx, solution, eval_delay=eval_delay)

        glob_mlu , glob_solution , glob_link_loads = self.optimal_preconfigured_path_routing_mlu(tm_idx)

        #norm_mlu = optimal_mlu / mlu
        #line = str(tm_idx) + ', ' + str(norm_mlu) + ', ' + str(mlu) + ', ' 


        print('opt',optimal_mlu)
        print(opti_link_loads)
        print('cri',crit_mlu)
        print(crit_link_loads)
        print('top',topk_mlu)
        print(topk_link_loads)
        print('ecm',topk_mlu)
        print(ecmp_link_loads)
        print('glb',glob_mlu)
        print(glob_link_loads)

        print('======================================')

        singleton.opti_link_loads.append(opti_link_loads)
        singleton.crit_link_loads.append(crit_link_loads)
        singleton.topk_link_loads.append(topk_link_loads)
        singleton.glob_link_loads.append(glob_link_loads)
        singleton.ecmp_link_loads.append(ecmp_link_loads)
        singleton.glob_solutions.append(glob_solution)


        line = '('+str(optimal_mlu) + '), ' 
    ...
    def cal_sol_load(sol):
        ...
        for idx,o in enumerate(sol):
            ...
            for i,oi in enumerate(o):
                for e in singleton.sdi2edges[(s,d,i)]:
                    loads[singleton.edge2idx[e]] += oi*singleton.traffic_matrices[s][d]
